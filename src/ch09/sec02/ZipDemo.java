...
public class ZipDemo {
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        Path zipFile = javaHome.resolve("lib/src.zip");
        if (!Files.exists(zipFile)) {
            System.err.println(zipFile + " does not exist.");
            return;
        }

        String sourceName = "java.base/java/lang/String.java";
        ...
        try (FileSystem zipfs = FileSystems.newFileSystem(zipFile)) {
            ...
            System.out.println("\nFiles in the jdk.zipfs module");
            try (Stream<Path> entries = Files.walk(zipfs.getPath("/"))) {
                entries.filter(p -> Files.isRegularFile(p) && p.toString().startsWith("/jdk.zipfs/"))
